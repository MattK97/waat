...
import '../../../../main.dart';
import 'calendar_chosen_day_schedule_pill_no_border.dart';

class CalendarTodayBuilder extends ConsumerWidget {
  final DateTime date;

  CalendarTodayBuilder({this.date});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final schedule = schedulesN.scheduleList.firstWhere(
        ...
        orElse: () => null);
    if (schedule == null) {
      return CalendarChosenDaySchedulePillNoBorder(date, Colors.redAccent, BoxDecoration());
    } else {
      return schedule.confirmation
          ? _calendarTodayBuilderChildWidget(schedule)
          : Opacity(
              child: _calendarTodayBuilderChildWidget(schedule),
              opacity: 0.5,
            );
    }
  }

  Widget _calendarTodayBuilderChildWidget(Schedule schedule) {
    if (schedule.holiday) {
