...
import 'calendar_schedule_pill.dart';

class CalendarHolidayBuilder extends ConsumerWidget {
  final DateTime date;

  CalendarHolidayBuilder({this.date});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (schedule == null) {
      ...
      return schedule.confirmation
          ? _calendarHolidayBuilderChildWidget(schedule)
          : Opacity(
              child: _calendarHolidayBuilderChildWidget(schedule),
              opacity: 0.5,
            );
    }
  }

  Widget _calendarHolidayBuilderChildWidget(Schedule day) {
    if (day.holiday) {
      return CalendarSchedulePill(
        date,
        Colors.white,
        BoxDecoration(
